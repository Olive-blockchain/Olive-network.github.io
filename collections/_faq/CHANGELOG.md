...
- You can now specify which private key to use for `olive plots create`. After obtaining the fingerprint from `olive keys show`, try `olive plots create -a FINGERPRINT`. Thanks to @eFishCent for this pull request!
...
- All appropriate olivelisp smart transactions have been updated to use aggsig_me.
...
- olivevdf now allows passing in input to a VDF for new consensus.
- sha256tree has been removed from olivelisp.
...
- Tests now place a cache of blocks in the ~/.olive/ directory to speed up total testing time.
...
- A segfault caused by memory leaks in bls-library has been fixed. This should end the random farmer and harvester crashes over time as outlined in [Issue 500](https://github.com/olive-Network/olive-blockchain/issues/500).
...
- FreeBSD and OpenBSD should be able to build and install olive-blockchain and its dependencies again.
...
- In the GUI there was [a regression](https://github.com/olive-Network/olive-blockchain/issues/484) that removed the scroll bar on the Plot page. The scroll bar has returned!
...
- Logging had [a regression](https://github.com/olive-Network/olive-blockchain/issues/485) where log level could no longer be set by service.

## [1.0beta16] aka Beta 1.16 - 2020-10-20

### Added

- The olive GUI now supports dark and light mode.
- The GUI now supports translations and localizations. If you'd like to add your language you can see the examples in [the locales directory](https://github.com/olive-Network/olive-blockchain/tree/dev/electron-react/src/locales) of the olive-blockchain repository.
- `olive check plots` now takes a `-g` option that allows you to specify a matching path string to only check a single plot file, a wild card list of plot files, or all plots in a single directory instead of the default behavior of checking every directory listed in your config.yaml. A big thank you to @eFishCent for this pull request!
...
- Updated to olivepos 0.12.32. This update significantly speeds up the F1/first table plot generation. It also now can log disk usage while plotting and generate graphs. More details in the [olivepos release notes](https://github.com/olive-Network/olivepos/releases/tag/0.12.32).
...
- olivepos 0.12.32 fixed a an out of bounds read that could crash the plotter. It also contains a fix to better handle the case of drive letters on Windows.
...
- olivevdf now has full IFMA optimizations for processors that support it.

### Changed

- Multithreading support in olivepos, as well as a new algorithm which is faster and does 70% less IO. This is a significant improvement in speed, much lower total writing, and configurability for different hardware environments.
...
- The rpc interfaces of all olive services have been refactored, simplified, and had various additional functionality added.
- Block timestamps are now stored in the wallet database. Both database versions were incremented and databases from previous versions will not work with Beta 14. However, upon re-sync all test olive since Beta 12 should appear in your wallet.
...
- olivepos issue where memory was spiking and increasing
- Fixed working space estimates so they are exact
- Log all errors in olivepos
...
- Long_description_content_type is now set to improve olive-blockoliven's Pypi entry. Thanks to @altendky for this pull request.
...
- You can now backup your wallet related metadata in an encrypted and signed file to a free service from Olive Blockchain at backup.olive.net. Simply having a backup of your private key will allow you to fully restore the state of your wallet including coloured coins, rate limited wallets, distributed identity wallets and many more. Your private key is used to automatically restore the last backup you saved to the olive backup cloud service. This service is open source and ultimately you will be able to configure your backups to go to backup.olive.net, your own installation, or a third party's version of it.
- Added a Code of Conduct in CODE_OF_CONDUCT.md.
- Added a bug report template in `.github/ISSUE_TEMPLATE/bug_report.md`.

### Changed

- This is a new blockchain as we changed how the default puzzle hashes are generated and previous coins would not be easy to spend. Plots made with Beta 8 and newer continue to work, but all previous test olive are left on the old chain and do not migrate over. Configuration data like plot directories automatically migrate in your `~/.olive` directory.
...
- `olive init` now automatically discovers previous releases in each new release.
...
- In Windows the start menu item is now Olive Blockchain and the icon in Add/Remove is updated.

## [1.0beta11] aka Beta 1.11 - 2020-08-24

### Added

- The olive UI now has a proper About menu entry that gives the various component versions and directs people to submit issues on GitHub. Thank you to @freddiecoleman for this pull request!
- Ability to run only the farmer, wallet, or timelord services, for more advanced configurations (olive run farmer-only, wallet-only, timelord-only)
...
- `olive show -w` now correctly outputs all wallet types and balances from the local wallet.
...
- `olive show -b` now returns plot public key and pool public key for each block.
- Added cbor2 binary wheels for ARM64 to the olive simple site. Raspberry Pi should be just a little easier to install.

### Changed

- Wallet addresses and other key related elements are now expressed in Chech32 which is the olive implementation of [Bech32](https://github.com/bitcoin/bips/blob/master/bip-0173.mediawiki). All of your old wallet addresses will be replaced with the new Chech32 addresses. The only thing you can't do is send test olive between 1.8/1.9 and 1.10 software. Anyone who upgrades to 1.10 will keep their transactions and balances of test olive from the earlier two releases however.
...
- The proof of space document in olivepos has been updated to the new format and edited for clarity. Additionally GitHub actions now has the on demand ability to create the PDF version.
...
- `olive show -w` now displays a message when balances cannot be displayed instead of throwing an error. Thanks to @freddiecoleman for this fix!
- Fix issue with shutting down full node (full node processes remained open, and caused a spinner when launching olive)
- Various code review alerts for comparing to a wider type in olivepos were fixed. Additionally, unused code was removed from olivepos
...
- See wallet balances in command line: `olive show -w`
- Retry opening invalid plots every 20 minutes (so you can copy a large plot into a plot directory.)
- We've added `olive keys sign` and `olive keys verify` to allow farmers to certify their ownership of keys.
...
- `olive start wallet-server` changed to `olive start wallet`, for consistency.
...
- Removed legacy scripts such as olive-stop-server, olive-restart-harvester, etc.
...
are read.
- Updated [blspy](https://github.com/olive-Network/bls-signatures) to use the
...
- The various sub repositories (olivepos, olivevdf, etc.) now build ARM64 binary
...
- The olive executable is now available if installing from the Windows or MacOS
Graphical installer. Try `./olive -h` from
`~\AppData\Local\olive-Blockchain\app-0.1.8\resources\app.asar.unpacked\daemon\`
in Windows or
`/Applications/olive.app/Contents/Resources/app.asar.unpacked/daemon` on MacOS.
...
"olive plots create" and "olive plots check".
- Add plot directories to config.yaml from the cli with "olive plots add".
...
- Linux builds of olivevdf and blspy now use a fresh build of gmp 6.2.1.
...
- The Windows installer is now signed by a Olive Blockchain certificate. It may take some time to develop enough reputation to not warn multiple times during install.
...
- Test plots for the simulation and testing harness now go into `~/.olive/test-plots/`
- We have completely refactored all networking code towards making each olive service use the same default networking infrastructure and move to websockets as the default networking wire protocol.
- We added additional improvements and more RPCs to the start daemon and various services to continue to make olive start/stop reliable cross platform.
...
- We now codesign the Apple .dmg installer with the Olive Blockchain developer ID on both GitHub Actions and Azure Pipelines. We will be notarizing and distributing the Azure Pipelines version as it's built on MacOS Mojave (10.14.6) for stronger cross version support.
...
- Starting olive now waits for the full node to be active before contacting the introducer.
...
- You can now create or import private keys with a 24 word mnemonic, both in the UI and 'olive keys' command line.
...
- We added a `olive keys` command line program, to see, add, and remove private keys.
...
- We added [lgtm](https://lgtm.com/) source analysis on pull request to the olive-blockchain, olivepos, olivevdf, olivebip158, and bls-library repositories to add some automated security analysis to our ci.
...
- `olive netspace` has been refactored to use the `/get_network_space` RPC. The command
...
- olivepos is now easier to compile with MSVC.
...
- `olive-check-plots` uses the plot root and checks the plots in the same manner as harvester.
- `olive-check-plots` now does not override plots.yaml, which means concurrent plots will properly be added to plots.yaml.
...
- We have added an option to `olive-create-plots` to specify the second temporary directory. Creating a plot is a three step process. First a working file ending in `.dat.tmp` is created. This file is usually 5 times larger than the final plot file. In the later stages of plotting a second temp file is created ending in `.dat.2.tmp` which will grow to the size of the final plot file. In the final step, the `.dat.2.tmp` is copied to the final `.dat` plot file. You can now optionally set the directory for the `.dat.2.tmp` file with the `-2` flag. An example use case is plotting on a ramdisk and writing both the second temp file and the final file out to an SSD - `olive-create-plots -n 1 -k 30 -t /mnt/ramdisk -2 /mnt/SSD -d /mnt/SSD`.

### Changed

- `olive init` properly migrates from previous versions including the k>=32 workaround. Additionally, the farming target key is checked to make sure that it is the valid and correct public key format.
- We have implemented a workaround for the `olive start` issues some were having upon crash or reboot. We will be rebuilding start and stop to be robust across platforms.
- This release re-includes `olive-start-harvester`.
...
- The user interface now refers to olive coins with their correct currency code of XCH.
...
- Building the sub reposities (olivepos, olivevdf, blslibrary) now requires CMake 3.14+.

### Fixed

- There was a regression in olive Proof of Space ([olivepos](https://github.com/olive-Network/olivepos)) that came from our efforts to speed up plotting on Windows native. Now k>=32 plots work correctly. We made additional bug fixes and corrected limiting small k size generation.
...
- Now you can run estimates of the total space currently farming the network. Try `olive netspace -d 12` to run an estimate over the last 12 blocks which is approximately 1 hour.
- We’ve added TLS authentication for incoming farmer connections. TLS certs and keys are generated during olive init and only full nodes with your keys will be able to connect to your Farmer. Also, Harvester, Timelord, and Wallet will now not accept incoming connections which reduces the application attack surface.
- The node RPC has a new endpoint get_header_by_height which allows you to retrieve the block header from a block height. Try `olive show -bh 1000` to see the block header hash of block 1000. You can then look up the block details with `olive show -b f655e1a9f7f8c89a703e40d9ce82ae33508badaf7b37fa1a56cad27926b5e936` which will look up a block by it's header hash.
...
- Most of the content of README.md and INSTALL.md have been moved to the [repository wiki](https://github.com/olive-Network/olive-blockchain/wiki) and placed in [INSTALL](https://github.com/olive-Network/olive-blockchain/wiki/INSTALL) and [Quick Start Guide](https://github.com/olive-Network/olive-blockchain/wiki/Quick-Start-Guide)
...
- Most scripts have been removed in favor of olive action commands. You can run `olive version` or `olive start node` for example. Just running `olive` will show you more options. However `olive-create-plots` continues to use the hyphenated form. Also it's now `olive generate keys` as another example.
- olive start commands like `olive start farmer` and `olive stop node` now keep track of process IDs in a run/ directory in your configuration directory. `olive stop` is unlikely to work on Windows native for now. If `olive start -r node` doesn't work you can force the run/ directory to be reset with `olive start -f node`.
- We suggest you take a look at our [Upgrading documentation](https://github.com/olive-Network/olive-blockchain/wiki/Updating-beta-software) if you aren't performing a new install.
...
- The script `olive start wallet-gui` has been chaned to `olive start wallet` which launches but the GUI and server on MacOS and Linux. `olive start wallet-server` remains for WSL 2 and Windows native.
...
- Plots of k>=32 are not working for farming, and some broken plots can cause a memory leak. A [workaround is available](https://github.com/olive-Network/olive-blockchain/wiki/Beta-1.4-k=32-or-larger-work-around).
...
- Currently, there is no way to restore a Coloured Coin Wallet.
- The `olive stop all` command sometimes fails, use `olive-stop-all` instead. In windows, use the task manager to stop the servers.
...
- We added full block lookup to `olive show`.

### Changed

- `olive-restart-harvester` has been renamed from `olive-start-harvester` to better reflect its functionality. Use it to restart a harvester that's farming so that it will pick up newly finished plots.
...
- If you previously used the plot_root variable in config, your plot directory names might not migrate correctly. Please double check the filenames in `~/.olive/beta-1.0b3/config/plots.yaml` after migrating
...
- There is now full transaction support on the olive blockchain. In this initial Beta 1.0 release, all transaction types are supported though the wallets and UIs currently only directly support basic transactions like coinbase rewards and sending coins while paying fees. UI support for our [smart transactions](https://github.com/olive-Network/wallets/blob/main/README.md) will be available in the UIs shortly.
...
- `install-timelord.sh` has been pulled out from `install.sh`. This script downloads the source python package for olivevdf and compiles it locally for timelords. vdf_client can be included or excluded to make building normal user wheels easier.

### Removed

- The Beta release is not compatible with the history of the Alpha blockchain and we will be ceasing support of the Alpha chain approximately two weeks after the release of this Beta. However, your plots and keys are fully compatible with the Beta chain. Please save your plot keys! Examples of how to save your keys and upgrade to the Beta are available on the [repo wiki](https://github.com/olive-Network/olive-blockchain/wiki).
- The ssh ui and web ui are removed in favor of the cli ui and the Electron GUI. To mimic the ssh ui try `olive show -s -c` and try `olive show --help` for usage instructions.
...
- olive fullnode, farmer and harvester now default to logging to olive.log in the olive-blockchain directory. This is configured in config.yaml and due to config.yaml changes it is recommended to edit the new template config instead of using older config.yaml’s from previous versions.
...
- Changed the displayed process names for harvester, farmer, fullnode, timelords, and VDFs to to olive_full node, olive_harvester, etc. Fixed a bug that could cause inadvertent shutdown of other processes like an ongoing plotting session when new olive services were started.
...
- This is the first release of the olive testnet! Blockchain consensus, proof of time, and proof of space are included.
- More details on the release at [https://www.olive.net/developer/](https://www.olive.net/developer/)

[unreleased]: https://github.com/olive-Network/olive-blockchain/compare/1.0beta5...dev
[1.0beta5]: https://github.com/olive-Network/olive-blockchain/compare/1.0beta4...1.0beta5
[1.0beta4]: https://github.com/olive-Network/olive-blockchain/compare/1.0beta3...1.0beta4
[1.0beta3]: https://github.com/olive-Network/olive-blockchain/compare/1.0beta2...1.0beta3
[1.0beta2]: https://github.com/olive-Network/olive-blockchain/compare/1.0beta1...1.0beta2
[1.0beta1]: https://github.com/olive-Network/olive-blockchain/compare/alpha-1.5.1...1.0beta1
[alpha 1.5.1]: https://github.com/olive-Network/olive-blockchain/compare/alpha-1.5...alpha-1.5.1
[alpha 1.5]: https://github.com/olive-Network/olive-blockchain/compare/alpha-1.4.1...alpha-1.5
[alpha 1.4.1]: https://github.com/olive-Network/olive-blockchain/compare/alpha-1.4...alpha-1.4.1
[alpha 1.4]: https://github.com/olive-Network/olive-blockchain/compare/alpha-1.3...alpha-1.4
[alpha 1.3]: https://github.com/olive-Network/olive-blockchain/compare/alpha-1.2...alpha-1.3
[alpha 1.2]: https://github.com/olive-Network/olive-blockchain/compare/alpha-1.1.1...alpha-1.2
[alpha 1.1.1]: https://github.com/olive-Network/olive-blockchain/compare/alpha-1.1...alpha-1.1.1
[alpha 1.1]: https://github.com/olive-Network/olive-blockchain/compare/alpha-1.0...alpha-1.1
[alpha 1.0]: https://github.com/olive-Network/olive-blockchain/releases/tag/Alpha-1.0
